...
/*
 * SUSFS domain identification helpers.
 * These are called from fs/susfs.c (added by the kernel-side SUSFS patch)
 * to let SUSFS ask "is the current process zygote / zygote_next / ksu / init"
 * without SUSFS having to know about KernelSU's internal SID cache.
 */
...
static u32 susfs_ksu_sid __read_mostly = 0;
static u32 susfs_init_sid __read_mostly = 0;
u32 susfs_zygote_sid __read_mostly = 0;
u32 susfs_zygote_next_sid __read_mostly = 0;
static u32 susfs_priv_app_sid __read_mostly = 0;
...
static inline void susfs_set_sid(const char *secctx_name, u32 *out_sid)
{
	...
	if (!secctx_name || !out_sid) {
		pr_err("susfs_set_sid: secctx_name || out_sid is NULL\n");
		return;
	}

	err = security_secctx_to_secid(secctx_name, strlen(secctx_name), out_sid);
	if (err) {
		pr_err("susfs_set_sid: failed setting sid for '%s', err: %d\n", secctx_name, err);
		return;
	}
	pr_info("susfs_set_sid: sid '%u' is set for secctx_name '%s'\n", *out_sid, secctx_name);
}
...
u32 susfs_get_sid_from_name(const char *secctx_name)
{
	...
	if (!secctx_name) {
		pr_err("susfs_get_sid_from_name: secctx_name is NULL\n");
		return 0;
	}
	err = security_secctx_to_secid(secctx_name, strlen(secctx_name), &out_sid);
	if (err) {
		pr_err("susfs_get_sid_from_name: failed getting sid from secctx_name: %s, err: %d\n", secctx_name, err);
		return 0;
	}
	return out_sid;
}
